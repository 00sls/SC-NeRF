...
def batchify(fn, chunk):
    ...
    def ret(inputs, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir):
        rgbs, depths = [], []
        for i in range(0, inputs.shape[0], chunk):
            rgb, depth = fn(inputs[i:i + chunk], dists, depth_candidates, feat_sampled, ndc_3d, rays_dir)
            rgbs.append(rgb)
            depths.append(depth)
        return torch.cat(rgbs, 0), torch.cat(depths, 0)

    return ret

def run_network_mvs(pts, viewdirs, alpha_feat, fn, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir, embed_fn, embeddirs_fn, netchunk=1024):
    ...
    outputs_flat, depth = batchify(fn, netchunk)(pts, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir)
...
def raw2outputs(raw, z_vals, dists, white_bkgd=False, net_type='v2'):
    ...
    return rgb_map, acc_map, weights, alpha, depth_map
...
def rendering(args, feat_sampled, ndc_3d, pose_ref, rays_pts, rays_ndc, depth_candidates, rays_o, rays_dir,
    ...
    # print(ndc_3d.shape)
    # print("---------------")
    # torch.Size([3, 3096, 128, 35])
    # torch.Size([3096, 128, 3])
    # using direction
    ...
    del imgs, volume_feature, img_feat
    dists = depth2dist(depth_candidates, cos_angle)
    # rays_ndc = rays_ndc * 2 - 1.0
    raw, depth_map_fn = network_query_fn(rays_ndc, angle, input_feat, network_fn, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir)
    if raw.shape[-1]>4:
        input_feat = torch.cat((input_feat[...,:8],raw[...,4:]), dim=-1)

    
    # dists = ndc2dist(rays_ndc)
    rgb_map, acc_map, weights, alpha, depth_map = raw2outputs(raw, depth_candidates, dists, white_bkgd,args.net_type)
    ret = {}
    # print(rgb_map.shape)
    # print(depth_map.shape)
    # print(depth_map_fn.shape)
    # print("------------")
    return rgb_map, input_feat, weights, depth_map, depth_map_fn, alpha, ret
