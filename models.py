...
from renderer import run_network_mvs, raw2alpha
...
class Renderer_ours(nn.Module):
    def __init__(self, args, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, input_ch_feat=8, skips=[4], use_viewdirs=False):
        """
        """
        ...
        self.embed, _ = get_embedder(args.multires, args.i_embed, input_dims=args.pts_dim)
        self.embeddirs_fn, _ = get_embedder(args.multires_views, args.i_embed, input_dims=args.dir_dim)
        self.embed_fn, input_ch = get_embedder(5, 0, 1)
        self.embed_fn_view, input_ch = get_embedder(5, 0, 3)
        ...
        self.feature_embed = nn.Linear(20, W)
        self.mulheadatt = MultiHeadAttention(4, 128, 8, 8)
        self.mulheadatt2 = MultiHeadAttention(4, 128, 8, 8)
        self.input_feature_embed = nn.Linear(35, W)
        self.depth_dir = nn.Linear(44, W)
        self.input_views_feat = nn.Linear(33, W)
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W)
            self.alpha_linear1 = nn.Sequential(nn.Linear(W, W),
                                               nn.ReLU(),
                                               nn.Linear(W, W),
                                               nn.ReLU(),
                                               nn.Linear(W, 1),
        ...
        self.alpha_linear1.apply(weights_init)
        self.rgb_linear.apply(weights_init)
        self.mulheadatt.apply(weights_init)
        self.input_feature_embed.apply(weights_init)
        self.mulheadatt2.apply(weights_init)
        self.feature_embed.apply(weights_init)
        self.input_views_feat.apply(weights_init)
    def forward(self, x, dists, depth_candidates, input_feature, ndc_3d, rays_dir):
        dim = x.shape[-1]
        ...
        input_feats_embed = self.feature_embed(input_feats)
        h = input_pts
        ...
        alpha = self.alpha_linear1(h)      
        # h, att = self.mulheadatt1(input_feats_embed, h, h)
        # print(alpha.shape)
        alpha_out, weights, alpha_softmax = raw2alpha(alpha[..., 0], dists, 'v0')
        depth_map = torch.sum(weights * depth_candidates, -1)
        # print(depth_map.shape)  # torch.Size([3096])
        depth_map_emb = depth_map
        near, far = torch.min(depth_map_emb), torch.max(depth_map_emb)
        depth_map_emb = 2*torch.pi*((depth_map_emb-near)/(far-near+0.0001))  # 归一化并转化为0~2*pi
        depth_map_emb = depth_map_emb.unsqueeze(1).repeat(1, h.shape[1]).unsqueeze(2).contiguous()
    
        depth_map_emb = self.embed_fn(depth_map_emb)  # torch.Size([3096, 128, 1])
        input_views = self.embed_fn_view(input_views)
        views_feat = self.input_views_feat(input_views)  # 新增
        V, N, S, _ = input_feature.shape
        input_feature = torch.mean(input_feature,dim=0)  # torch.Size([3096, 128, 35])
        input_feature = self.input_feature_embed(input_feature)
        depth_dir = self.depth_dir(torch.cat([input_views, depth_map_emb], -1))
        
        h = h.permute(0, 2, 1)
        views_feat = views_feat.permute(0, 2, 1)
        input_feats_embed = input_feats_embed.permute(0, 2, 1)
        h, _ = self.mulheadatt(views_feat, input_feats_embed, h)
        del input_feats_embed, views_feat
        depth_dir = depth_dir.permute(0, 2, 1)
        input_feature = input_feature.permute(0, 2, 1)
        h, _ = self.mulheadatt2(depth_dir, input_feature, h)
        del depth_dir, input_feature
        h = h.permute(0, 2, 1)
        h = self.feature_linear(h)  # 128 - 128
        for i, l in enumerate(self.views_linears):
            h = self.views_linears[i](h)   # 128+11+33 - 
            h = F.relu(h)
...
class MVSNeRF(nn.Module):
    def __init__(self, args, D=8, W=256, input_ch_pts=3, input_ch_views=3, input_ch_feat=8, skips=[4], net_type='v2'):
        """
        """
        ...
        if 'v0' == net_type:
            self.nerf = Renderer_ours(args, D=D, W=W,input_ch_feat=input_ch_feat,
                     input_ch=input_ch_pts, output_ch=4, skips=skips,
                     input_ch_views=input_ch_views, use_viewdirs=True)
        


    def forward(self, x, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir):
        RGBA, depth = self.nerf(x, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir)
        return RGBA, depth
...
def create_nerf_mvs(args, pts_embedder=True, use_mvs=False, dir_embedder=True):
    ...
    skips = [4]
    model = MVSNeRF(args, D=args.netdepth, W=args.netwidth,
    ...
    network_query_fn = lambda pts, viewdirs, rays_feats, network_fn, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir: run_network_mvs(pts, viewdirs, rays_feats, network_fn, dists, depth_candidates, feat_sampled, ndc_3d, rays_dir, 
                                                                        embed_fn=embed_fn,
    ...
    if len(ckpts) > 0 :
        ...
        if ckpt_path.split('/')[-1] == 'mvsnerf-v0.tar':
            ckpt = torch.load(ckpt_path)
            if use_mvs:
                state_dict = ckpt['network_mvs_state_dict']
                EncodingNet.load_state_dict(state_dict)

            print("load mvsnerf-v0.tar")
        else:
            ckpt = torch.load(ckpt_path)

            # Load model mvsnerf_depth/ckpts/mvsnerf-v0.tar
            if use_mvs:
                state_dict = ckpt['network_mvs_state_dict']
                EncodingNet.load_state_dict(state_dict)
    ...
    def __init__(self, in_channels, norm_act=InPlaceABN):
        ...
        self.conv11 = nn.Sequential(
            nn.ConvTranspose3d(16, 8, 3, padding=1, output_padding=1,
                               stride=2, bias=False),
            norm_act(8))
        self.channel_att = eca_block(5, 1, 2)
